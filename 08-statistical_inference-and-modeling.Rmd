...
The `ttest_ind()` function performs a t test between two arrays specified with the `a` and `b` arguments, respectively. By default the function assumes equal population variances. By setting the `equal_var` parameter to False we can relax that assumption and run Welch's t test, which is the default in R. The function returns the test statistic and p-value.

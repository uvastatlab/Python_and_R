...
import statsmodels.api as sm
import matplotlib.pyplot as plt
import statsmodels.graphics.regressionplots as grf

log_reg = smf.logit("low ~ age + lwt + race + smoke + ptd + ht + ui + ftv", data=birthwt).fit()

#sm.graphics.plot_fit(log_reg,2)
plt.clf()
sm.graphics.plot_fit(log_reg,"age")
plt.show()
```
...
## TRIAL
###########
x = birthwt.drop(['low'], axis = 1)
x = pd.get_dummies(x, columns=['race','ftv'], drop_first=True).astype('float64')
y = birthwt["low"].values.astype('float64')
x = sm.add_constant(x)

log_reg_TRIAL = sm.Logit(y,x)
log_reg_TRIAL_fit = log_reg_TRIAL.fit()

sm.graphics.plot_fit(log_reg_TRIAL_fit,4)
plt.show()
...
plt.clf()
ax = sns.regplot(y = "low", x = "lwt", data = birthwt, logistic = True)
plt.show()
```

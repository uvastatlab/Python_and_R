...
Oftentimes functions have built-in error-checking that return messages describing the error. Here we show a simple error-check to ensure that the argument passed to our function is a number. 
...
Python functions can return more than one result. It will output the results into a `tuple`. At tuple is a data structure very similar to a list, but it is immutable - we cannot change the order of the entries. Here we make our function return both the mean and the SEM of our data. 
...
_Centering_ refers to rescaling a column or vector of values such that their mean is 0. This is sometimes performed to aid interpretation of linear model coefficients. 

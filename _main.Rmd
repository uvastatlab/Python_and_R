...
The `lower()`, `upper()`, and `title()` functions convert case to lower, upper, and title, respectively. We can use a list comprehension to apply these functions to each word in a list.
...
The `title` function changes all fo the
col_names = [col.upper() for col in mtcars.columns]
...
The numpy function `random.choice`()` can be used to sample from a DataFrame.

...
The Python statistics library `statsmodels` has a function `GLM` that fits a general linear model using a specified model. The two main arguments of this function are `endog` and `exog`. `endog` refers to the endogenous or dependent variable, and `exog` refers to the exogenous or independent variable(s) in the dataset. 
...
We can also see coefficients by using the `.params` attribute of our model, and confidence intervals using the `.conf_int()` method. 

```{python}
res.params #coefficients of the model 
res.conf_int() #confidence intervals of the model, defaults to 95% but can specify in the argument
```
...
```{python}
sm.graphics.plot_fit(log_reg,"low" )
```
```{python}
type(log_reg)
```

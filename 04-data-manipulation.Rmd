...
## Find and replace patterns within strings

#### Python

#### R

The base R functions `sub()` and `gsub()` can be used to find and replace the first or all instances of a pattern in strings, respectively. The pattern searched for can be expressed as a raw character string or as a regular expression.
```{r}
statements <- c('Great Pencil Co. sells pencils of the following grades: HB; B; and 3B.',
...
```{r}
spaced_string <- c('There      are    too   many    spaces     in this   string')
...
The package **stringi** also provides an array of string manipulation functions, including `stri_detect()`, `stri_replace()`, and `stri_extract()`, all of which easily handle fixed and regex search patterns. For example:
```{r}
library(stringi)

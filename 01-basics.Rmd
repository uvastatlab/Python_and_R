...
The mathematical operators for addition (`+`), subtraction (`-`), multiplication (`*`), and division (`/`) are the same in both languages. However, the operators for exponentiation, integer division, and remainder division (modulo) differ:
...
To see the value of an object, you can enter the object name in the console and hit enter in both Python and R. It is common in Python, however, to explicitly use the `print()` function.
...
Functions can be bundled and distributed as packages for installation. After installation, they're "imported" (Python) or "attached" (R) in each new session when needed.

Packages with large user bases are often updated to add functionality and fix bugs. Updates are not automatically installed. Staying apprised of library/package updates can be challenging; to stay up to date, you might consider signing up for newsletters, following developers on social media, or periodically checking to see what updates are available.

Packages often depend on other packages. These interconnections are known as "dependencies." Packages are sometimes updated to accommodate changes to packages they depend on.

#### Python {-}

When you download Python, you gain access to the Python Standard Library. This library includes several data types and functions for storing data, performing mathematical operations, and beyond. Commonly used data types include _lists_, _tuples_, and _sets_. You do not need to import data types from the Python Standard Library.
...
Libraries contain modules---groups of functions collected in `.py` scrips. To use functions from modules in the Python Standard Library, users must import the appropriate module. Examples include **math** and **itertools**.
...
Users can download hundreds of libraries (a broader term for packages and other bundles of functionally related software) beyond the Python Standard Library. Popular libraries include **NumPy**, which provides functions for operating on arrays/vectors, and **pandas**, which provides functions for data analysis. The following code gives an example of importing a Python library, **NumPy**, into a Python script, and using one of its functions to create an array (a data structure discussed in Chapter 2).
...
The following screenshot illustrates how to install a library to an environment using the Anaconda GUI. Using the drop down menu, navigate to "Not installed." Then, select the desired library from the list. (You can use the search bar to search for libraries by name.) Finally, click "Apply" to install the package.
...
To install R packages from GitHub, use the `install_github()` function from the **devtools** package. You need to include the username of the repo owner followed by a forward slash and the name of the package. Typing two colons between a package name and a function in the package allows you to use that function without attaching the package in your R environment. That's how we access `install_github()` in the example below.
...
**Section should be rewritten/clarified with input from our Python folks: `&` and `|` behavior isn't directly comparable. R converts arguments to & and | to logical values and then evaluates. Python performs actual bitwise operations (e.g., PY `4 & 4` gives `4`; R `4 & 4` gives `TRUE`. I rewrote the short-circuiting section.**
...
Both languages have "short-circuiting" operators: `and` and `or` in Python; `&&` and `||` in R. Short-circuit operators only evaluate a set of logical expressions until a result is determined (proceeding left to right). For example: The first line of R code below (using `&`) fails because the right-hand expression throws an error, so the left-hand and right-hand expressions can't be jointly evaluated for truth. The second line, however (using `&&`) succeeds because the code _short-circuits_ after evaluating the left-hand expression: Since `(1 == 2)` is `FALSE`, the entire line must be false, and the right-hand material is not evaluated.

```{r, eval = F}
(0 == 1) & (sum('0', 1) == sum(0, 1))
# sum('0', 1) can't be computed, so the whole line fails

(0 == 1) && (sum('0', 1) == sum(0, 1))
# sum('0', 1) is never computed, because (0 == 1) is FALSE, so the whole statement is therefore FALSE
```

Python and R have different operators for negation: Python uses `not`; R uses `!`. For exclusive or (XOR), Python uses `^` (e.g., `True^False`), whereas R uses `xor()` (e.g., `xor(TRUE, FALSE)`).

#### Python {-}

Python's relational operators can be used to compare arrays to single values or other arrays. The following operation returns a **NumPy** array containing `True` and `False` values. 
...
We can make multiple comparisons with the `and` and `or` operators. For a compound expression created with the `and` operator to return `True`, all component expressions must be true; for a compound expression created with the `or` operator to return `True`, one or more of the component expressions must be `True`.
...
`TRUE`/`FALSE` values in R have numeric values of 1 and 0, respectively. This allows us to perform mathematical operations with logical values. (For example, an average of `TRUE` (`1`) and `FALSE` (`0`) values is a proportion.)

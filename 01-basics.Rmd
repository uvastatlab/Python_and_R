...
Functions in Python are defined by using the `def` keyword followed by the name we choose for our function with its arguments inside parentheses. Here we create a function to calculate the standard error of a mean (SEM) and call it `SEM`. 

```{python}
def SEM(x):
  import numpy as np
  s = np.std(x) # find standard deviation of the input array

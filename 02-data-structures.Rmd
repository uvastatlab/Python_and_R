...
Pandas Series also need to be declared using the `pandas.Series()` function. Like `numpy`, the `pandas`package must be imported as well. The pandas package is built on numpy, so we can input data into a pandas Series using a numpy array. 
```{python}
import pandas as pd 
import numpy as np

data1 = np.array([1,2,3,"hello"])
ser1 = pd.Series(data2)
print(ser1)
```

We can change the indices of the Series to have values other than the traditional 0, 1, 2, etc. to whatever we like using the `index` attribute within the `Series` function. We can then use our own specified indices to select and index our data.

```{python}
import pandas as pd 
import numpy as np

data2 = np.array([1,2,3,"hello"])
ser2 = pd.Series(data2, index=['a','b','c','d'])
print(ser2)
```
